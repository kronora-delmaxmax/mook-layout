...
> 定位：**网页版是测试模型**——验证体验与技术（AI 识别、版式、视频合成），不做商业化；付费、账号、支付都在原生版解决。网页版 Pro 模拟解锁是刻意设计，供测试者体验完整功能。

## 网页版（测试优先）
- [ ] 微信 X5 内核兼容性实测 + 必要时加「用浏览器打开」引导（用户主要从微信点链接进来）
- [ ] AI 识别 / 抠图 iPhone 真机验证；66MB 模型首次加载的进度提示
- [x] 视频不进草稿的刷新提示（放入时 toast 说明）
- [ ] 分享链接 OG 预览卡片（meta 标签 + 封面图，零成本提升传播）
- [ ] PWA（manifest + Service Worker）：添加到主屏幕、离线可用，最低成本逼近原生体验
- [ ] 产品方向待定：一页流（现状）vs 多页翻页杂志——决定产品骨架，尽早拍板
